        ...
        try:
            ...
            try:
                ...
                if img.width < 100 or img.height < 100:
                    if os.path.exists(file_path):
                        os.remove(file_path)
                    return JsonResponse({'match_found': False, 'error': 'Image too small for recognition'})
                
                # Check if image is too blurry (optional)
                # This is a simple check - you might want to implement a more sophisticated blur detection
            except Exception as e:
                pass  # Continue even if we can't check image quality
            ...
            for student in students:
                ...
                if stored_encoding is not None:
                    try:
                        ...
                        # Collect match information for debugging
                        matches_info.append({
                        ...
                        if distance < 0.45 and distance < best_distance:  # Even stricter threshold
                            best_distance = distance
                            best_match = student
                            match_count += 1
                    except Exception as e:
                        # print(f"Error comparing faces for {student.name}: {e}")
                        continue
            
            # Only return a match if we have a good confidence level
            if best_match and best_distance < 0.45:  # Even stricter threshold
                # Convert file path to URL
                if best_match.photo_path and settings.MEDIA_ROOT in best_match.photo_path:
                    photo_url = best_match.photo_path.replace(settings.MEDIA_ROOT, settings.MEDIA_URL.rstrip('/')).replace('\\', '/')
                else:
                ...
                return JsonResponse({
                    ...
                    'confidence': round(1 - best_distance, 3),  # Higher value means better match
                    'distance': round(best_distance, 3)
                })
            else:
                # Sort matches by distance for debugging
                matches_info.sort(key=lambda x: x['distance'])
                top_matches = matches_info[:5]  # Top 5 closest matches
                
                return JsonResponse({
                    'match_found': False, 
                    'error': 'No matching student found',
                    'best_distance': round(best_distance, 3) if best_distance != float('inf') else None,
                    'top_matches': top_matches  # For debugging
                })
